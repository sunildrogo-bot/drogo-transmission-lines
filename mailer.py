...
def mail_configuration():
    """Read sender configuration at send time; never expose the password."""
    ...
    try:
        ...
        port = 587
    return {'host': os.environ.get('SMTP_HOST', 'smtp.gmail.com'), 'port': port,
            'username': username, 'password': password,
            'sender': os.environ.get('SMTP_FROM') or username,
            'name': os.environ.get('MAIL_SENDER_NAME', 'Drogo Aerospace')}


def _is_configured() -> bool:
    config = mail_configuration()
    return bool(config['username'] and config['password'] and config['sender'])
...
if _is_configured():
    print("[mailer] Welcome email sender configured.")
else:
    print(
