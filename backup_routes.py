...
@backup_bp.route('/api/settings/offsite-backups', methods=['GET', 'POST'])
def offsite_backups():
    guard = _admin_guard()
    if guard:
        return guard
    from offsite_backup import configuration

    config = configuration()
    query = BackgroundJob.query.filter_by(job_type='offsite_backup')
    if request.method == 'POST':
        if not config['configured']:
            return jsonify({
                'error': 'Offsite backup is not configured. Set BACKUP_OFFSITE_ENABLED=true and BACKUP_OFFSITE_BUCKET.'
            }), 409
        active = query.filter(BackgroundJob.status.in_(['Queued', 'Processing'])).first()
        if active:
            return jsonify(active.to_dict()), 200
        from background_jobs import enqueue
        job = enqueue('offsite_backup', {}, session.get('user_id'), session.get('user_name', 'Admin'))
        return jsonify(job.to_dict()), 202

    jobs = query.order_by(BackgroundJob.id.desc()).limit(10).all()
    return jsonify({'configuration': config, 'jobs': [job.to_dict() for job in jobs]})
