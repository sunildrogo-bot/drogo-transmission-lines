...
function installBackupManager() {
  ...
  wrap.innerHTML = `<div class="settings-card">
      ...
      <div class="backup-actions"><button class="btn-ghost" id="data-health-btn" onclick="loadDataHealth(true)">Check Data Health</button>
        ...
        <button class="btn-ghost" id="backup-upgrade-btn" onclick="createApplicationBackup(true)">Create Pre-Upgrade Backup</button>
        <button class="btn-primary" id="offsite-backup-btn" onclick="startOffsiteBackup()">Create Offsite Checkpoint</button></div>
      <div class="backup-message" id="offsite-backup-summary">Checking offsite backup configuration…</div>
    </div>
    <div class="section-label">LOCAL BACKUP HISTORY</div><div id="backup-history" class="backup-list"><div class="backup-message">Loading backup history…</div></div>`;
  loadOffsiteBackupStatus();
}

async function loadOffsiteBackupStatus() {
  const target = document.getElementById('offsite-backup-summary');
  const button = document.getElementById('offsite-backup-btn');
  if (!target || !button) return;
  try {
    const response = await fetch('/api/settings/offsite-backups');
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Offsite backup status failed.');
    const config = data.configuration || {};
    const latest = (data.jobs || [])[0];
    button.disabled = !config.configured || Boolean(latest && ['Queued','Processing'].includes(latest.status));
    if (!config.configured) {
      target.className = 'backup-message warn';
      target.textContent = 'Offsite backup is not configured. Local database checkpoints do not protect the upload volume from VPS disk failure.';
      return;
    }
    target.className = 'backup-message';
    const destination = `${config.bucket}/${config.prefix}`;
    if (!latest) {
      target.textContent = `Offsite destination ${destination} · retention ${config.retention_count} checkpoints · no checkpoint created yet.`;
      return;
    }
    const result = latest.result || {};
    const verified = result.restore_verified ? ' · restore verified' : '';
    target.textContent = `Offsite destination ${destination} · latest ${latest.status}${verified} · ${result.phase || 'waiting'}`;
    clearTimeout(window.offsiteBackupPollTimer);
    if (['Queued','Processing'].includes(latest.status)) {
      window.offsiteBackupPollTimer = setTimeout(loadOffsiteBackupStatus, 3000);
    }
  } catch (error) {
    target.className = 'backup-message warn';
    target.textContent = error.message;
...
async function startOffsiteBackup() {
  const button = document.getElementById('offsite-backup-btn');
  if (button) { button.disabled = true; button.textContent = 'Queued…'; }
  try {
    const response = await fetch('/api/settings/offsite-backups', {method:'POST'});
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Could not queue offsite backup.');
    await loadOffsiteBackupStatus();
  } catch (error) {
    const target = document.getElementById('offsite-backup-summary');
    target.className = 'backup-message warn';
    target.textContent = error.message;
  } finally {
    if (button) button.textContent = 'Create Offsite Checkpoint';
  }
}
...
async function loadDataHealth(forceMessage = false) {
  ...
  try {
    ...
    const archivedNote = data.files.archived_raw_reference_count
      ? `${Number(data.files.archived_raw_reference_count).toLocaleString()} intentionally deleted raw photo reference(s) were safely excluded from missing-file checks.`
      : '';
    ...
    message.innerHTML = (notes.length
      ...
      + (archivedNote ? `<div class="backup-message">${escapeHtml(archivedNote)}</div>` : '');
