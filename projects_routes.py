...
def _save_upload(file_storage, subfolder, allowed_exts):
    ...
    base_dir = os.path.realpath(current_app.static_folder)
    folder_fs = os.path.join(base_dir, 'uploads', subfolder)
...
def _generate_thumbnail(static_root, subfolder_raw, filename):
    """Create a high-quality grid thumbnail without touching the original."""
    from PIL import Image, ImageOps
    raw_dir = os.path.normpath(os.path.join(static_root, 'uploads', subfolder_raw))
    raw_full_path = os.path.join(raw_dir, filename)
    thumb_dir = os.path.join(os.path.dirname(raw_dir), 'thumb')
    thumb_subfolder = os.path.relpath(thumb_dir, os.path.join(static_root, 'uploads')).replace('\\', '/')
...
def _generate_flat_thumbnail(static_root, flat_subfolder, filename):
    ...
    from PIL import Image, ImageOps
    raw_full_path = os.path.join(static_root, 'uploads', flat_subfolder, filename)
    thumb_subfolder = f"{flat_subfolder}_thumb"
    thumb_dir = os.path.join(static_root, 'uploads', thumb_subfolder)
    thumb_name = secure_filename(filename) + '.thumb.jpg'
    thumb_full_path = os.path.join(thumb_dir, thumb_name)
    temp_path = thumb_full_path + '.tmp'
    try:
...
def _save_tower_photo(file_storage, project, division, line, tower_label, allowed_exts, kind='raw'):
    ...
    folder_fs = os.path.join(static_root, 'uploads', subfolder)
    os.makedirs(folder_fs, exist_ok=True)
    safe_name = secure_filename(file_storage.filename)
    name_root, name_ext = os.path.splitext(safe_name)
    final_name = safe_name
    ...
    thumb_path = None
    if kind == 'raw':
        thumb_path = _generate_thumbnail(static_root, subfolder, final_name)
        if thumb_path:
...
def api_thermal_probe(photo_id):
    ...
    try:
        ...
        path = _storage_working_path(photo.display_image_path())
...
def _build_project_defect_summary(project):
    ...
    for defect in defects:
        ...
        defect_rows.append({
            ...
            'image_path': photo.display_image_path(),
            'image_url': stored_url(photo.display_image_path()),
...
def project_defect_report(project_id):
    ...
    try:
        ...
        for row in summary['defect_rows']:
            _storage_working_path(row.get('image_path'))
...
def _ensure_line_geojson_cache(line):
    ...
    cache_name = f'line_{line.id}_{uuid.uuid4().hex}.geojson'
    destination = os.path.join(folder, cache_name)
    temporary = destination + '.tmp'
    with open(temporary, 'w', encoding='utf-8') as handle:
        json.dump(data, handle, separators=(',', ':'), ensure_ascii=False)
    os.replace(temporary, destination)
    line.geojson_path = f'uploads/kml_cache/{cache_name}'
    from storage_service import get_storage
...
def api_edit_kml(line_id):
    ...
    try:
        if full_path.lower().endswith('.kmz'):
            with zipfile.ZipFile(full_path) as archive:
                kmz_entries = [(info, archive.read(info)) for info in archive.infolist()]
            kml_name = next(info.filename for info, _ in kmz_entries if info.filename.lower().endswith('.kml'))
            tree = ET.ElementTree(ET.fromstring(next(content for info, content in kmz_entries if info.filename == kml_name)))
        else:
            tree = ET.parse(full_path)
    except (ET.ParseError, zipfile.BadZipFile, StopIteration) as e:
    ...
    # Immutable keys prevent another worker or object-storage cache from
    # serving the previous geometry after a successful edit.
    extension = '.kmz' if kmz_entries is not None else '.kml'
    edited_key = f'uploads/kml_edits/line_{line.id}_{uuid.uuid4().hex}{extension}'
    edited_path = os.path.join(current_app.static_folder, edited_key)
    os.makedirs(os.path.dirname(edited_path), exist_ok=True)
    if kmz_entries is not None:
        with zipfile.ZipFile(edited_path, 'w') as archive:
            for info, content in kmz_entries:
                archive.writestr(info, ET.tostring(tree.getroot(), encoding='utf-8', xml_declaration=True)
                                 if info.filename == kml_name else content)
    else:
        tree.write(edited_path, encoding='utf-8', xml_declaration=True)
    get_storage().publish(edited_key, edited_path)
    line.kml_path = edited_key
    line.geojson_path = None
    auto_count = _count_kml_tower_points(edited_path)
    if auto_count is not None:
        line.tower_count = auto_count
    _ensure_line_geojson_cache(line)
    db.session.commit()
...
def _reopen_tower_for_new_photo(line_id, tower_label):
    # New evidence requires a fresh tower-level SME release, in this transaction.
    TowerInspectionStatus.query.filter_by(line_id=line_id, tower_label=tower_label).update(
        {'inspection_done': False, 'marked_by': '', 'marked_at': None}, synchronize_session='fetch')
    AiInspectionSummary.query.filter_by(line_id=line_id, tower_label=tower_label).update(
        {'is_shared': False}, synchronize_session='fetch')
...
def api_upload_tower_photo(line_id):
    ...
    _reopen_tower_for_new_photo(line_id, tower_label)
    db.session.commit()
...
def api_tower_defects_flat(line_id):
    ...
    for d in defects:
        entry = d.to_dict()
        entry['image_url'] = stored_url(d.photo.display_image_path())
...
def api_tower_thermal_points_flat(line_id):
    ...
    for pt in points:
        entry = pt.to_dict()
        entry['image_url'] = stored_url(pt.photo.display_image_path())
        out.append(entry)
...
def api_list_thermal_points(photo_id):
    ...
    image_abs_path = _storage_working_path(photo.display_image_path())
