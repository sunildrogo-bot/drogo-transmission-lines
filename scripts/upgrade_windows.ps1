...
try {
    ...
    if (Test-Path -LiteralPath $database -PathType Leaf) {
        ...
        & $python "$incomingApp\scripts\snapshot_sqlite.py" `
            --source $database --destination $databaseBackup
        if ($LASTEXITCODE -ne 0) { throw 'SQLite pre-upgrade backup verification failed.' }
    } else {
