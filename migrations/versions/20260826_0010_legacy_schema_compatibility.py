...
def upgrade():
    ...
    if 'reset_token' in _columns('users') and 'ix_users_reset_token' not in _indexes('users'):
        op.create_index('ix_users_reset_token', 'users', ['reset_token'], unique=False)
    if 'thumbnail_path' not in _columns('corridor_photos'):
        op.add_column('corridor_photos', sa.Column('thumbnail_path', sa.String(255), nullable=True))
    if 'defect_copy_path' not in _columns('tower_photos'):
        op.add_column('tower_photos', sa.Column('defect_copy_path', sa.String(255), nullable=True))
    if 'raw_deleted' not in _columns('tower_photos'):
        op.add_column('tower_photos', sa.Column('raw_deleted', sa.Boolean(), nullable=True, server_default=sa.false()))
    if 'thumbnail_path' not in _columns('tower_photos'):
        op.add_column('tower_photos', sa.Column('thumbnail_path', sa.String(255), nullable=True))
    if 'resolved_by' not in _columns('tower_defects'):
        op.add_column('tower_defects', sa.Column('resolved_by', sa.String(120), nullable=True))
    if 'resolved_at' not in _columns('tower_defects'):
        op.add_column('tower_defects', sa.Column('resolved_at', sa.DateTime(), nullable=True))
    if 'resolution_comment' not in _columns('tower_defects'):
        op.add_column('tower_defects', sa.Column('resolution_comment', sa.Text(), nullable=True))
