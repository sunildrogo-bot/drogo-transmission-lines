...
def create_app(db_url: str | None = None) -> Flask:
    instance_override = os.environ.get('INSTANCE_PATH', '').strip()
    app = Flask(__name__, instance_path=os.path.abspath(instance_override)) if instance_override else Flask(__name__)

    # ── Config ────────────────────────────────────────────────────────────────
    app.secret_key = os.environ.get('SECRET_KEY', DEFAULT_DEVELOPMENT_SECRET)
    ...
    app.config['SESSION_COOKIE_SECURE'] = env_flag(
        'SESSION_COOKIE_SECURE', default=is_production(app.config['DEPLOYMENT_ENV']))
    ...
    def pilot_line_work(line_id):
        if session.get('role') != 'Pilot':
            return redirect(url_for('client_dashboard'))
        from models import Line, PilotAssignment
...
if __name__ == '__main__':
    development = not is_production(app.config.get('DEPLOYMENT_ENV'))
    app.run(
        host=os.environ.get('APP_HOST', '127.0.0.1'),
        port=int(os.environ.get('APP_PORT', '5000')),
        debug=development and env_flag('FLASK_DEBUG', default=False),
    )
