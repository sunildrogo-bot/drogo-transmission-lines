...
def _save_user_photo(file_storage):
    ...
    if ext not in PHOTO_EXTS:
        return None
    folder_fs = os.path.join(os.path.realpath(current_app.static_folder), 'uploads', 'user_photos')
...
def api_create():
    ...
    new_user['_email_status'] = 'sent' if email_sent else ('failed' if mailer._is_configured() else 'not_configured')
    if not email_sent:
...
@users_bp.route('/api/users/email-status', methods=['GET'])
def email_status():
    guard = _require_admin()
    if guard:
        return guard
    configured = mailer._is_configured()
    return jsonify({'configured': configured,
                    'message': ('New users receive a password setup email automatically. SMTP delivery is checked when sent.'
                                if configured else 'Email sender is not configured. Set GMAIL_ADDRESS and GMAIL_APP_PASSWORD (or SMTP credentials) in .env, plus PUBLIC_BASE_URL, and restart the application.')})
