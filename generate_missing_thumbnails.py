...
from projects_routes import _generate_flat_thumbnail, _generate_thumbnail
    ...
    with app.app_context():
        ...
        for photo in photos:
            if not photo.image_path:
                continue
            full_path = os.path.join(static_root, photo.image_path)
            ...
            thumb_path = _generate_thumbnail(static_root, subfolder_raw, filename)
            if thumb_path:
        ...
        for photo in corridor_photos:
            full_path = os.path.join(static_root, photo.image_path or '')
            if not os.path.isfile(full_path):
                corridor_missing += 1
                continue
            ...
            thumb_path = _generate_flat_thumbnail(static_root, flat_subfolder, filename)
            if thumb_path:
                photo.thumbnail_path = thumb_path
